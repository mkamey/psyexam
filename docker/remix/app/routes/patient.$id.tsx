...
export default function PatientExams() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
      ...
      <h1 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">Patient Exam Results</h1>
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md w-full max-w-md">
        {stackedExams.length > 0 ? (
          <ul className="space-y-2">
            {stackedExams.map((exam) => (
              <li key={exam.id} className="p-3 border border-gray-200 dark:border-gray-700 rounded-md bg-gray-50 dark:bg-gray-700 flex justify-between items-center">
                <span className="text-gray-900 dark:text-gray-100">{exam.exam.examname}</span>
                <Link
          ...
          <p className="text-gray-500 dark:text-gray-400">No exams found for this patient.</p>
        )}
      </div>
    </div>
  );
